...
DEVANA-KEY: src/api/backend.ts:5049 | fulfill-partial-persist
DEVANA-SUMMARY: fixed | P1 | high | fulfillPaidOrder now persists FORWARD PROGRESS: if a line fails mid-loop, the already-fulfilled lines (carrying their stockMovementId/entitlementId) are written to the order ledger WITHOUT a fulfilledAt marker before rethrowing, so committed per-line goods are always recorded in a durable order record and never orphaned. This closes the multi-line "poison line" gap an adversarial review surfaced after an initial (wrong) invalid call: L1's committed stock+entitlement were left with NO matching order once a deterministically-failing L2 exhausted the bounded retries. Compensating rollback was rejected (it would revoke a paid customer's goods); a dead-letter/auto-refund disposition for permanently-unfulfillable lines is a scoped follow-up.