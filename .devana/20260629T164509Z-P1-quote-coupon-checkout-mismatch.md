...
DEVANA-KEY: src/api/backend.ts:5814 | quote-coupon-checkout-mismatch
DEVANA-SUMMARY: fixed | P1 | high | checkout.start (and checkout.preview, via the shared StartCheckoutInput base) now accept couponCode and apply it — persisting to the backing cart like applyCoupon — so checkout charges the coupon the shopper was quoted instead of a stale persisted one.