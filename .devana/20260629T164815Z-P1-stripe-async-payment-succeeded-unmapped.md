...
- 2026-06-30: fixed (chosen approach exactly as the report's Suggested Next Step: add the success mapping in `parseStripeWebhookEvent`). The missing branch was the whole bug — `checkout.session.async_payment_succeeded` matched none of the existing branches and fell through to `unknownStripeWebhookEvent`, so a genuinely-paid delayed-notification order (SEPA/ACH/bank transfer/BACS/boleto) was silently dropped (`receiveWebhook` returns 200 `received`, no order, no fulfillment, no email). Fix: widened the existing paid-Checkout branch (`src/stripe.ts`, previously `type === "checkout.session.completed" && stripeCheckoutSessionIsPaid(object)`) to ALSO match `type === "checkout.session.async_payment_succeeded"`, returning the identical paid `payment` event shape — crucially carrying `providerCheckoutId = stringChild(object,"id")` (the `cs_…` session id) plus `providerPaymentId`/`providerOrderId` from the `payment_intent`. Design choice (and WHY): the async success is matched BY EVENT TYPE, NOT gated on `payment_status` — symmetric with how the FAILURE side already keys off the type alone (`checkout.session.async_payment_failed` ∈ `STRIPE_PAYMENT_FAILURE_TYPES`, no status check). Stripe's contract is that `async_payment_succeeded` ⟹ funds settled, so trusting the type (rather than re-checking `payment_status:"paid"`) is the robust choice: it can never re-introduce the silent under-fulfillment failure mode for an edge where the echoed session object's `payment_status` is absent/lagging. Downstream is UNCHANGED and shared with the settled-payment path (confirmed against this report's Counterevidence): a paid `payment` event with `providerCheckoutId` → `processPaymentWebhook` → `findPaymentEventCheckout` resolves the still-pending checkout document by the session id (the earlier unpaid `checkout.session.completed` having been correctly dropped means no order exists yet) → the order is created and fulfilled exactly as for a settled `checkout.session.completed`. No `src/api/backend.ts` change was needed. Evidence: a new parse-level test in `test/acp-stripe.test.ts` (modeled on the existing `async_payment_failed` normalization test) drives a realistic `checkout.session.async_payment_succeeded` payload through `verifyWebhook` + `parseWebhookEvent` and asserts (via `expectPaidProviderPaymentEvent` + `toMatchObject`) `kind:"payment"`, `paymentStatus:"paid"`, `type:"checkout.session.async_payment_succeeded"`, `providerCheckoutId:"cs_async_succeeded"`, `providerPaymentId`/`providerOrderId:"pi_async"`, customer email, and `totals`. Mutation-verified: with the new condition neutered (cp-backup + restore, no git), the test FAILS with `kind:"unknown"` — the exact reported silent-drop behavior; restored via cp and re-confirmed green. Full suite (374) and both tsc configs pass. Out of scope (NOT changed): the unpaid `checkout.session.completed` event still maps to `unknown` (correct — the order is created later when the async success arrives); the report's refutation that `payment_intent.succeeded` cannot cover this (it never sets `providerCheckoutId`, so `findPaymentEventCheckout` returns null → `markWebhookFailed`) was re-confirmed and that branch was left untouched; an end-to-end fulfillment test was judged disproportionate since the post-parse path is byte-for-byte the already-covered settled-Checkout path — the classification fix is the fix.

DEVANA-KEY: src/stripe.ts:746 | stripe-async-payment-succeeded-unmapped
DEVANA-SUMMARY: fixed | P1 | high | parseStripeWebhookEvent now maps checkout.session.async_payment_succeeded to a paid payment event (by event type, symmetric with async_payment_failed) carrying providerCheckoutId, so delayed-payment (SEPA/ACH/etc.) Checkout orders link to their pending checkout and fulfill instead of being silently dropped as unknown. Downstream fulfillment path is shared with settled checkout.session.completed; no backend change needed.
