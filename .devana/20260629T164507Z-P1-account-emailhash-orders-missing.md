...
- 2026-06-30: reopened and fixed. The first regression included an entitlement, so it did not cover paid guest orders that have no entitlement rows. `resolveAccountIdentity` now accepts a verified session email hash when `ledger.listOrdersByEmailHash(emailHash, 1)` finds a matching order, and a new regression covers the order-only guest identity. Empty/deleted email-hash sessions still do not resolve because they have neither active entitlements nor matching orders.

DEVANA-KEY: src/api/backend.ts:1233 | account-emailhash-orders-missing
DEVANA-SUMMARY: fixed | P1 | high | Orders are now indexed by emailHash and account.get lists guest orders for emailHash-only magic-link identities, including order-only guests without entitlement rows.
