...
- 2026-06-30: binding-completeness caveat addressed. Preview and start now use one explicit delegated-payment proof projection that includes cart/sellable identity, provider, couponCode, customer, redirects, quote totals/items, and sanitized custom fields. The volatile delegated metadata keys (`acpPaymentToken`, `acpPaymentAuthorizationInputHash`) are stripped before hashing, so preview does not need a token while start can carry it to the provider. Regression verifies changing a non-volatile custom field after preview fails 403, while the same preview hash succeeds when start only adds the delegated token/hash fields; provider input still receives the token metadata.
- 2026-06-30: follow-up review found the real ACP complete flow also adds handoff metadata (`acpPaymentProvider`, generated `acpPaymentAuthorizationId`, and `acpCheckoutSessionId`) and passes the buyer as checkout customer after preview. The proof sanitizer now treats those ACP handoff identifiers as volatile, and ACP preview uses the same buyer/customer that complete sends to `checkout.start`. Regressions cover backend acceptance when only ACP handoff identifiers are added after preview and handler-level preview customer alignment.

DEVANA-KEY: src/api/backend.ts:6914 | checkout-proof-not-enforced
DEVANA-SUMMARY: invalid | P1 | high | checkout.start already enforced delegated payment authorization; the remaining binding-completeness caveat is now closed by a shared preview/start proof projection covering coupon/customer/redirect/custom fields while stripping volatile delegated token metadata from the hash.
