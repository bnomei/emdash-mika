...
DEVANA-KEY: src/stripe.ts:746 | provider-refund-chargeback-webhook-unmapped
DEVANA-SUMMARY: fixed | P2 | medium | Stripe reversal webhooks downgrade orders off paid and full reversals revoke access. Redelivered full reversals now also re-run idempotent revocation for already-refunded orders, so a prior ledger-success/revoke-failure attempt cannot permanently leave entitlements or licenses active.
