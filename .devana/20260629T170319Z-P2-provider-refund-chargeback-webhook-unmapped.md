...
- 2026-06-30: reopened and fixed for a bad idempotency edge. The first fix marked an already-`refunded` order as processed without revoking access. If the first webhook attempt wrote the refunded order and then `revokeOrderFulfillmentAccess` failed, a provider redelivery skipped revocation permanently. The already-refunded branch now re-runs the idempotent revocation before acknowledging. A new dispatcher regression forces the first entitlement revocation write to throw, verifies the order is `refunded` while access remains active, then redelivers the webhook and asserts entitlement/license access is revoked.
- 2026-06-30: fixed the out-of-order reversal edge. A reversal carrying a strong provider payment/order id but no matching order is now marked failed/replayable instead of processed, so provider redelivery can apply it after the paid checkout webhook creates the order. Weak uncorrelated reversals still ack as processed to avoid retrying non-Mika provider noise forever. Regression: reversal arrives first (`failed`), paid webhook creates the order, the same reversal redelivery reprocesses the failed webhook and moves the order to `refunded`.

DEVANA-KEY: src/stripe.ts:746 | provider-refund-chargeback-webhook-unmapped
DEVANA-SUMMARY: fixed | P2 | medium | Stripe reversal webhooks downgrade orders off paid and full reversals revoke access. Redelivered full reversals re-run idempotent revocation for already-refunded orders, and strong unmatched reversal events now fail retryably until the matching order exists instead of being silently acknowledged.
