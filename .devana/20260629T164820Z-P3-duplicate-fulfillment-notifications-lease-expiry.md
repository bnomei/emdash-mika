...
DEVANA-KEY: src/api/backend.ts:5107 | duplicate-fulfillment-notifications-lease-expiry
DEVANA-SUMMARY: open | P3 | medium | On workflow lease expiry mid-fulfillment, a re-leased worker re-emits `license.issued`/`download.ready` host notifications, which (unlike the confirmation email) have no idempotency guard, so they double-fire.
