...
- 2026-06-30: fixed the post-fulfilled retry gap. If the final `fulfilledAt` ledger write succeeded and `download.ready` failed afterward, a replay saw an already-fulfilled order and returned after order confirmation, leaving the failed download-ready marker unretried. `fulfillPaidOrder` now still runs idempotent post-fulfilled side effects for already-fulfilled orders: order confirmation and `download.ready` with existing refs included. The marker still dedupes successful retries. Regression forces the first `download.ready` hook to throw after `fulfilledAt`, then redelivers the webhook and asserts the same marker completes without duplicate later emissions.

DEVANA-KEY: src/api/backend.ts:5107 | duplicate-fulfillment-notifications-lease-expiry
DEVANA-SUMMARY: fixed | P3 | medium | license.issued/download.ready emit through marker workflows, and already-fulfilled webhook retries still run idempotent post-fulfilled side effects so a prior fulfilledAt-success/download-ready-failure attempt can self-heal on redelivery without duplicate successful emissions.
