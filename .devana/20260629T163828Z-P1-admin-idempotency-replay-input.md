...
- 2026-06-30: type cleanup. The audit start record now types `idempotencyInput` as `JsonValue`; replay snapshots persisted under audit metadata are only stored/read as `JsonObject`; `runAdminAction` keeps concrete DTO result inference while casting only at the audit persistence boundary. Typecheck covers the JSON boundary without weakening admin result DTO types.

DEVANA-KEY: src/api/backend.ts:2566 | admin-idempotency-replay-input
DEVANA-SUMMARY: fixed | P1 | medium | runAdminAction hashes JSON-typed client request input, returns 409 when an idempotency key is reused with different input, and only stores JSON-object replay snapshots while preserving concrete admin result DTO inference.
